                                                                     ...
                                                                     const std::string& settingsKey) throw(ConfigError)
{
  ...
  else
  {
    ...
    else
    {
        ...
        if (settings.has(BEGINSTRING))
        {
			auto value = settings.getString(BEGINSTRING);
			if (value == BeginString_FIX40) {
                isetting.write(settings.FIX40Cache().data(), settings.FIX40Cache().size());
			}
			else if (value == BeginString_FIX41) {
                isetting.write(settings.FIX41Cache().data(), settings.FIX41Cache().size());
			}
			else  if (value == BeginString_FIX42) {
                isetting.write(settings.FIX42Cache().data(), settings.FIX42Cache().size());
			}
			else  if (value == BeginString_FIX43) {
                isetting.write(settings.FIX43Cache().data(), settings.FIX43Cache().size());
			}
			else if (value == BeginString_FIX44) {
                isetting.write(settings.FIX44Cache().data(), settings.FIX44Cache().size());
			}
			else  if (value == BeginString_FIX50) {
                isetting.write(settings.FIX50Cache().data(), settings.FIX50Cache().size());
			}
			else  if (value == BeginString_FIX50SP1) {
                isetting.write(settings.FIX50SP1Cache().data(), settings.FIX50SP1Cache().size());
			}
			else  if (value == BeginString_FIX50SP2) {
                isetting.write(settings.FIX50SP2Cache().data(), settings.FIX50SP2Cache().size());
			}
            else  if (value == BeginString_FIXT11) {
                isetting.write(settings.FIXT11Cache().data(), settings.FIXT11Cache().size());
            }
        }
        pDD = ptr::shared_ptr<DataDictionary>(new DataDictionary(isetting, preserveMsgFldsOrder));
    }
