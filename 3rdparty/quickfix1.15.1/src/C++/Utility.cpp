...
{
  int socket = static_cast<int>(::socket( PF_INET, SOCK_STREAM, 0 ));
...
{
  return static_cast<int>(::socket( PF_INET, SOCK_STREAM, IPPROTO_TCP ));
}
...
quickfix_ssize_t socket_recv( int s, char* buf, size_t length )
{
  return recv( s, buf, static_cast<int>(length), 0 );
}

quickfix_ssize_t socket_send( int s, const char* msg, size_t length )
{
...
{
#ifdef _MSC_VER
  return reinterpret_cast<thread_id>(GetCurrentThread());
#else
  return pthread_self();
#endif
}
