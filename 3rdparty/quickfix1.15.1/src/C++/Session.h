...
namespace FIX
{
	...
	protected:
		IContext(const SessionID&);
		virtual ~IContext();
	public:
		bool Send(FIX::Message&);
		bool Ready() const;
		Session* SessionGet() const;
		const SessionID& SessionIDGet() const;
		const unsigned long long& ContextID() const;
	private:
		virtual void Release() const = 0;
	private:
		Session* m_pSession = nullptr;
		const SessionID m_SessionID;
		const unsigned long long m_ContextID;
	};
#endif
...
#if 1//!@ By Martell 20210311
		bool SendToTarget(Message&);
		static bool SendToTarget(const SessionID&, Message&);
		//!@ Push to all targets Acceptor private . . .
		//!@ Return push target count.
		static size_t PushToAllTargetSessions(Message&);
		static size_t PushToAllTargetContexts(Message&);
		static IContext* lookupContext(const SessionID&);
		static IContext* lookupContext(const unsigned __int64&);
		static void iterateSession(const std::function<void(Session*)>&);
		static void iterateContext(const std::function<void(IContext*)>&);
#endif
