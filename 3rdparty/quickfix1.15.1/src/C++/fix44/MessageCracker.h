...
{  
 ...
 virtual void onMessage( const Heartbeat&, const FIX::SessionID&)
  ...
  virtual void onMessage( const ConfirmationRequest&, const FIX::SessionID&)
    { throw FIX::UnsupportedMessageType(); }
	virtual void onMessage(const SourceRawData&, const FIX::SessionID&)
	{ throw FIX::UnsupportedMessageType(); }
...
public:
              ...
              const FIX::SessionID& sessionID)
  {
    ...
    if (msgTypeValue == "SRD")
      onMessage((const SourceRawData&)message, sessionID);
    else onMessage( message, sessionID);
  }
  
void crack( Message& message, 
            const FIX::SessionID& sessionID ,const FIX::Session* pSession)
  {
    FIX::MsgType msgType;
	...
	else if (msgTypeValue == "SRD")
		onMessage((SourceRawData&)message, sessionID);
    else onMessage( message, sessionID);
  }
