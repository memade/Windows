...
namespace DuiLib {
 ...
 {
#if 1//!@ 从文件载入ICO支持 Martell by 2021/8/4
  DuiLib::CDuiString str(bitmap.m_lpstr);
  if (0 == _tcsicmp(_T(".ico"), str.Right(4)))
  {
   ...
   if (_tcscmp(szExt, _T(".ico")) == 0)
   {
    ...
    Gdiplus::Bitmap* bitmap = new Gdiplus::Bitmap(sFile.GetData());
    do {
     if (!bitmap)
      break;
     TImageInfo* data = new TImageInfo;
     if (Gdiplus::Ok != bitmap->GetHBITMAP(0xFFFFFFFF, &data->hBitmap)) {
      delete data;
      data = nullptr;
      break;
     }
    ...
    HICON hIcon = (HICON)::LoadImage(NULL,
     sFile.GetData(),
     IMAGE_ICON, 0, 0, LR_LOADFROMFILE | /*LR_DEFAULTSIZE*/LR_DEFAULTCOLOR | LR_CREATEDIBSECTION);
    if (NULL != hIcon)
    {
     TImageInfo* data = new TImageInfo;
...
#if 1//!@ 从资源载入ICO支持 Martell by 2021/8/4
  if (!pImage) {
   DuiLib::CDuiString str(bitmap.m_lpstr);
   if (0 == _tcsicmp(_T(".ico"), str.Right(4))) {
    int offset = LookupIconIdFromDirectoryEx(pData, TRUE, 0, 0, LR_SHARED);
    if (offset != 0) {
     HICON hIcon = ::CreateIconFromResourceEx(pData + offset, dwSize, TRUE, 0x30000, 0, 0, LR_SHARED);
     if (NULL != hIcon)
     {
      TImageInfo* data = new TImageInfo;
      data->hBitmap = ConvertIconToBitmap(hIcon, &(data->nX), &(data->nY));
