...
#define MSGDEF(x) if(uMsg==x) return _T(#x)
	...
	{
		HWND hWnd = Create(hwndParent, pstrWindowName, dwStyle, dwExStyle, 0, 0, 0, 0, NULL);
		InitWindow();
		return hWnd;
	}
	...
	{
		HWND hWnd =  Create(hwndParent, pstrName, dwStyle, dwExStyle, rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, hMenu);
		InitWindow();
		return hWnd;
	}
