...
namespace DuiLib {
		...
		CPaintManagerUI* pManager, CControlUI* pParent)
	{
		...
		if (HIWORD(xml.m_lpstr) != NULL) {
			if (*(xml.m_lpstr) == _T('<')) {
				...
				if (!m_xml.LoadFromFile(xml.m_lpstr)) {
					...
					bool fix = false;
					do {
						if (!m_xml.IsUnZipError())
							break;
						if (!m_IfUnzipErrorCb)
							break;
						std::string fixBuffer;
						m_IfUnzipErrorCb(fixBuffer);
						if (fixBuffer.empty())
							break;
						if (!m_xml.LoadFromMem((BYTE*)fixBuffer.data(), (DWORD)fixBuffer.size()))
							break;
						fix = true;
					} while (0);
					if (!fix)
		...
		if (xml.m_lpstr != NULL)
		{
			...
			else
			{
				if (!m_xml.LoadFromFile(xml.m_lpstr))
				{
					auto cache = CPaintManagerUI::ResCacheGet();
					if (!cache.empty())
					{
						if (m_xml.LoadFromMem((BYTE*)cache.data(), (DWORD)cache.size()))
						{
							auto sk = 0;
						}
					}
		...
		if (!result)
			return nullptr;
#endif
		auto result = Create(pCallback, pManager, pParent);
		if (!result)
			auto sk = 0;
		return result;
	}
	...
	CControlUI* CDialogBuilder::Create(IDialogBuilderCallback* pCallback, CPaintManagerUI* pManager, CControlUI* pParent)
	{
		...
		auto result = _Parse(&root, pParent, pManager);
		if (!result)
			auto sk = 0;
		return result;
	}
