...
namespace DuiLib
{
	...
	CControlUI* CContainerUI::GetItemAtByTag(const UINT_PTR& tag) const {
		CControlUI* result = nullptr;
		bool found = false;
		for (int i = 0; i < m_items.GetSize(); ++i) {
			result = static_cast<CControlUI*>(m_items[i]);
			if (result) {
				if (result->GetTag() == tag) {
					found = true;
					break;
				}
			}
	...
	CControlUI* CContainerUI::GetItemAtByTagINT64(const INT64& tag) const {
		CControlUI* result = nullptr;
		bool found = false;
		for (int i = 0; i < m_items.GetSize(); ++i) {
			result = static_cast<CControlUI*>(m_items[i]);
			if (result) {
				if (result->GetTagINT64() == tag) {
					found = true;
					break;
				}
			}
	...
	CControlUI* CContainerUI::GetItemAtByTagUINT64(const UINT64& tag) const {
		CControlUI* result = nullptr;
		bool found = false;
		for (int i = 0; i < m_items.GetSize(); ++i) {
			result = static_cast<CControlUI*>(m_items[i]);
			if (result) {
				if (result->GetTagUINT64() == tag) {
					found = true;
					break;
				}
			}
