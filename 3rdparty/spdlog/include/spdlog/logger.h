...
typedef struct tagLogRoute final
{
    std::uint64_t type;
    ...
    tagLogRoute(const spdlog::level::level_enum &_level, const std::chrono::time_point<std::chrono::system_clock> &_time,
        const string_view_t &_logs, const std::uint64_t &_type = 0)
    {
        type = _type;
        level = _level;
        time = _time;
        text.append(_logs.data(), _logs.size());
    }
} LogRouteData, LogRoute;
using tf_spdlog_output_cb = std::function<void(LogRouteData&)>;
#endif
...
#if SPDLOG_ENABLE_EXTEND
public:
    void set_output_cb(const tf_spdlog_output_cb &msgcb)
    {
        m_output_cb = msgcb;
    }

private:
    tf_spdlog_output_cb m_output_cb = nullptr;
