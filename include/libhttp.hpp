...
#else
#if ENABLE_OPENSSL_FIPS
#pragma comment(lib,"libeaycompat32")
#else
...
#pragma pack(pop)
		...
		public:
			...
			const std::string& ContentBody() const;
			const std::string& ContentHead() const;
			bool LocationNextUrl(const std::function<void(const std::string&)>&) const;
			const std::string& DownCacheFilePathname() const;
