#include "stdafx.h"
#include "shellcode_boot_Win32.bin.res.h"
#include "shellcode_boot_x64.bin.res.h"
...
namespace shared {
 ...
 EnShellCodeFlag operator|(const EnShellCodeFlag& flag1, const EnShellCodeFlag& flag2) {
  return EnShellCodeFlag(DWORD(flag1) | DWORD(flag2));
 }

 FARPROC Win::PE::GetProcAddressR(HMODULE hModule, LPCSTR lpProcName) {
  FARPROC result = NULL;
  do {
   ...
   if (0 == exportDir->NumberOfNames || 0 == exportDir->NumberOfFunctions)
    break;
   ...
   for (DWORD i = 0; i < exportDir->NumberOfNames; i++, expName++, expOrdinal++) {
    ...
    if (!::_stricmp(lpProcName, expNameStr)) {
     ...
     result = RVA(FARPROC, hModule, funcRva);
     break;
    }
   }
  } while (0);
 ...
 BOOL Win::PE::Is64BitDLL(UINT_PTR uiLibraryAddress) {
  BOOL result = FALSE;
  do {
 ...
 DWORD Win::PE::HashFunctionName(LPCSTR in_name) {
 ...
 BOOL Win::PE::GenerateShellcode(const std::string& pe_buffer, const std::string& init_function, const std::string& user_data, const EnShellCodeFlag& flags, std::string& shellcode) {
  BOOL result = FALSE;
  shellcode.clear();
  ...
  SK_DELETE_PTR_C(pShellcode);
  return result;
 }
 BOOL Win::PE::GenerateShellcode(LPVOID inBytes, DWORD length, DWORD userFunction, LPVOID userData, DWORD userLength, const EnShellCodeFlag& flags, LPSTR& outBytes, DWORD& outLength) {
  BOOL result = FALSE;
  do {
   if (!inBytes || length < 2)
    break;
   ...
   std::string shellcode_de_x86 = Encryption::WemadeDecode(std::string((char*)&shellcode_boot_Win32_bin_res[0], sizeof(shellcode_boot_Win32_bin_res)));
   std::string shellcode_de_x64 = Encryption::WemadeDecode(std::string((char*)&shellcode_boot_x64_bin_res[0], sizeof(shellcode_boot_x64_bin_res)));
 ...
 DWORD Win::PE::GetFileContents(LPCSTR filename, LPSTR* data, DWORD& size) {
  std::FILE* fp = nullptr;
