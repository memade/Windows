...
#define PRIxPTR "x"
	...
	bool Win::PE::IsDLL(const std::string& pebuffer) {
		bool result = false;
		do {
			if (!PE::Verify(pebuffer))
				break;
			const size_t pesize = pebuffer.size();
			char* pBegin = const_cast<char*>(pebuffer.data());
			const IMAGE_DOS_HEADER* peDosHead = \
				reinterpret_cast<const IMAGE_DOS_HEADER*>(pebuffer.data());
			if (pesize < peDosHead->e_lfanew + sizeof(IMAGE_NT_HEADERS32))
				break;
			const IMAGE_NT_HEADERS32* peTempNtHead = \
				reinterpret_cast<const IMAGE_NT_HEADERS32*>(pBegin + peDosHead->e_lfanew);
			IMAGE_NT_HEADERS32* pe32NtHead = nullptr;
			IMAGE_NT_HEADERS64* pe64NtHead = nullptr;
			bool x64 = false;
			if (peTempNtHead->FileHeader.Machine == IMAGE_FILE_MACHINE_ARM64 || \
				peTempNtHead->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64) {
				x64 = true;
			}
			if (x64) {
				pe64NtHead = reinterpret_cast<IMAGE_NT_HEADERS64*>(pBegin + peDosHead->e_lfanew);
				if (pesize < peDosHead->e_lfanew + sizeof(IMAGE_NT_HEADERS64))
					break;
				//result = IMAGE_FILE_DLL == (pe64NtHead->FileHeader.Characteristics & IMAGE_FILE_DLL);
				result = 0x0000000180000000 == pe64NtHead->OptionalHeader.ImageBase;
			}
			else {//x86
				pe32NtHead = reinterpret_cast<IMAGE_NT_HEADERS32*>(pBegin + peDosHead->e_lfanew);
				if (pesize < peDosHead->e_lfanew + sizeof(IMAGE_NT_HEADERS32))
					break;
				//result = IMAGE_FILE_DLL == (pe32NtHead->FileHeader.Characteristics & IMAGE_FILE_DLL);
				result = 0x10000000 == pe32NtHead->OptionalHeader.ImageBase;
			}
		} while (0);
		return result;
	}
	bool Win::PE::IsEXE(const std::string& pebuffer) {
		bool result = false;
		do {
			if (!PE::Verify(pebuffer))
				break;
			...
			if (x64) {
				...
				if (pesize < peDosHead->e_lfanew + sizeof(IMAGE_NT_HEADERS64))
					break;
				//result = IMAGE_FILE_EXECUTABLE_IMAGE == (pe64NtHead->FileHeader.Characteristics & IMAGE_FILE_EXECUTABLE_IMAGE);
				result = 0x0000000140000000 == pe64NtHead->OptionalHeader.ImageBase;
			}
			else {//x86
				pe32NtHead = reinterpret_cast<IMAGE_NT_HEADERS32*>(pBegin + peDosHead->e_lfanew);
				if (pesize < peDosHead->e_lfanew + sizeof(IMAGE_NT_HEADERS32))
					break;
				//result = IMAGE_FILE_EXECUTABLE_IMAGE == (pe32NtHead->FileHeader.Characteristics & IMAGE_FILE_EXECUTABLE_IMAGE);
				result = 0x00400000 == pe32NtHead->OptionalHeader.ImageBase;
			}
		} while (0);
		return result;
	}
	DWORD Win::PE::PESize(const std::string& pe_buffer) {
		DWORD result = 0;
		do {
			if (!Verify(pe_buffer))
				break;
			bool x64 = IsX64(pe_buffer);

			PIMAGE_DOS_HEADER pImgDosHeader = (PIMAGE_DOS_HEADER)pe_buffer.data();
			if (!pImgDosHeader)
				break;
			void* pImgNtHeaders = nullptr;
			if (x64)
				pImgNtHeaders = (PIMAGE_NT_HEADERS64)(pe_buffer.data() + pImgDosHeader->e_lfanew);
			else
				pImgNtHeaders = (PIMAGE_NT_HEADERS32)(pe_buffer.data() + pImgDosHeader->e_lfanew);
			if (!pImgNtHeaders)
				break;
			PIMAGE_SECTION_HEADER pImgSecHeader = nullptr;
			if(x64)
				pImgSecHeader = (PIMAGE_SECTION_HEADER)((PIMAGE_NT_HEADERS64)pImgNtHeaders + 1);
			else
				pImgSecHeader = (PIMAGE_SECTION_HEADER)((PIMAGE_NT_HEADERS32)pImgNtHeaders + 1);
			if (!pImgSecHeader)
				break;
			if (x64)
				result = pImgDosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS64) + ((PIMAGE_NT_HEADERS64)pImgNtHeaders)->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER);
			else
				result = pImgDosHeader->e_lfanew + sizeof(IMAGE_NT_HEADERS32) + ((PIMAGE_NT_HEADERS32)pImgNtHeaders)->FileHeader.NumberOfSections * sizeof(IMAGE_SECTION_HEADER);
			// 0x200 对齐
			if (result % 0x200)
				result = (result / 0x200) * 0x200 + 0x200;
			if (x64) {
				for (int i = 0; i < ((PIMAGE_NT_HEADERS64)pImgNtHeaders)->FileHeader.NumberOfSections; i++)
					result += pImgSecHeader[i].SizeOfRawData;
			}
			else {
				for (int i = 0; i < ((PIMAGE_NT_HEADERS32)pImgNtHeaders)->FileHeader.NumberOfSections; i++)
					result += pImgSecHeader[i].SizeOfRawData;
			}
		} while (0);
		return result;
	}
	bool Win::PE::IsX64(const std::string& pebuffer) {
		bool result = false;
		do {
			if (!PE::Verify(pebuffer))
				break;
			...
			const IMAGE_NT_HEADERS32* peTempNtHead = \
				reinterpret_cast<const IMAGE_NT_HEADERS32*>(pBegin + peDosHead->e_lfanew);
			result = (peTempNtHead->FileHeader.Machine == IMAGE_FILE_MACHINE_ARM64 || \
				peTempNtHead->FileHeader.Machine == IMAGE_FILE_MACHINE_AMD64);
		} while (0);
		return result;
	}

	bool Win::PE::Verify(const std::string& pebuffer) {
		bool result = false;
		do {
			if (pebuffer.empty())
				break;
