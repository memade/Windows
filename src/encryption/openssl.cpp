...
namespace shared {
#if ENABLE_OPENSSL
 bool Encryption::MD5(const std::string input, std::string& output, const std::vector<std::uint8_t> seed /*= {}*/, const bool& hex_output /*= true*/) {
  bool result = false;
  output.clear();
#if ENABLE_OPENSSL_3
 ...
 bool Encryption::SHA256(const std::string& inData, const bool& isHex, std::string& outData) {
  bool result = false;
  outData.clear();
 ...
 bool Encryption::HMAC(const std::string& input, std::string& output, const std::string& seed, const bool& hex_output /*= true*/) {
  bool result = false;
#if ENABLE_OPENSSL_3
  do {
  ...
  do {
   ...
   if (!HMAC_Update(&hmacCtx, (const std::uint8_t*)input.data(), input.size()))
    break;
   output.resize(4096, 0x00);
   unsigned int md_size = 0;
   if (!HMAC_Final(&hmacCtx, (std::uint8_t*)&output[0], &md_size))
    break;
   if (md_size <= 0)
    break;
   output.resize(md_size);
   result = true;
  } while (0);
#endif
  if (!result)
 ...
 bool Encryption::Base64Encode(const std::string& input, std::string& output, const bool& multiline /*= false*/) {
  bool result = false;
  output.clear();
  BIO* bmem = nullptr;
  BIO* b64 = nullptr;
 ...
 bool Encryption::Base64Decode(const std::string& input, std::string& output, const bool& multiline /*= false*/) {
  bool result = false;
  output.clear();
  BIO* b64 = nullptr;
  BIO* bmem = nullptr;
 ...
 /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 bool Encryption::Rsa::Encode(const EnType& type, const std::string& pwd, const std::string& keyPublic, const std::string& keyPrivate, const std::string& inData, std::string& outData) {
  outData.clear();
  bool result = false;
  if (inData.empty())
   return false;
 ...
 bool Encryption::Rsa::Decode(const EnType& type, const std::string& pwd, const std::string& keyPublic, const std::string& keyPrivate, const std::string& inData, std::string& outData) {
  outData.clear();
  bool result = false;
  if (inData.empty())
   return false;
 ...
 bool Encryption::Rsa::DERPublic(const EnType& type, const std::string& keyPublic, const std::string& pwd, std::string& outDER) {
  bool result = false;
  outDER.clear();
  RSA* rsaObj = nullptr;
 ...
 bool Encryption::Rsa::PrivateToPublic(const EnType& type, const std::string& pwd, const std::string& inKeyPrivate, std::string& outKeyPublic) {
  bool result = false;
  outKeyPublic.clear();
 ...
 bool Encryption::Rsa::Generate(const EnType& type, const std::string& pwd, std::string& outKeyPublic, std::string& outKeyPrivate) {
  bool result = false;
  outKeyPublic.clear();
  outKeyPrivate.clear();
  RSA* rsaObj = nullptr;
